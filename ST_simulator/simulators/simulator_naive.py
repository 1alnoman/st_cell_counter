...
from utils.random_sampling_based_simu_utils import *
...
def get_sim_spot_level_expr( spot_num = None, spot_row = None, spot_col = None,
    ...
    if spot_generate_type == "square" and cell_coord_type == "generic":
        ...
        grid_row = np.ceil(image_width / spot_diameter).astype(int)+1
        grid_col = np.ceil(image_height / spot_diameter).astype(int)+1
...
def sim_naive_cell(use_real_adata=None,
              ...
              file_path=None,seed = 123,Min=0,Max=1):
    global simulatedat
    np.random.seed(seed)
    ...
    if method == 'RCTD':
        W,exp_spots2gene,spatial,spots2cell = RCTD_naive(scdata=use_real_adata,
                                                     ...
                                                     Max = Max,
                                                     ctkey=ctkey,
                                                     maxctnum = len(use_real_adata.obs[ctkey].unique()))
        exp_spots2gene = downsample_matrix_by_cell(exp_spots2gene,10000)
        simulatedat = sc.AnnData(csr_matrix(exp_spots2gene[:len(use_real_adata),:]))
        simulatedat.obs = pd.DataFrame(np.array(W[:len(use_real_adata),:] @ range(len(use_real_adata.obs[ctkey].unique())),dtype = int))
    ...
    if np.isin(method,['RCTD','STRIDE']):
        ...
        # simulatedat.obsm["spatial"] = use_real_adata.obsm.get('spatial', spatial[:len(use_real_adata),:])
...
def sim_naive_spot(use_real_adata=None,level='cell',spot_diameter=500,
                   ctkey=None,method=None,file_path=None,seed = 123,Min=0,Max=1):
    global sim_spot_adata
    sim_adata = sim_naive_cell(use_real_adata,ctkey,method,file_path,seed,Min,Max)
